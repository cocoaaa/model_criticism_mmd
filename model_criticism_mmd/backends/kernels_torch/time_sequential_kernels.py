...
class SoftDtwKernelFunctionTimeFeature(BasicRBFKernelFunction):
    ...
    def compute_kernel_matrix(self, x: torch.Tensor, y: torch.Tensor, **kwargs) -> KernelMatrixObject:
        ...
        __x = x_input.unsqueeze(0) if len(x_input.size()) == 2 else x_input
        __y = y_input.unsqueeze(0) if len(y_input.size()) == 2 else y_input

        soft_dt_xx = torch.pow(self.soft_dtw.forward(__x, __x, is_return_matrix=False), 2)
        soft_dt_xy = torch.pow(self.soft_dtw.forward(__x, __y, is_return_matrix=False), 2)
        soft_dt_yy = torch.pow(self.soft_dtw.forward(__y, __y, is_return_matrix=False), 2)
