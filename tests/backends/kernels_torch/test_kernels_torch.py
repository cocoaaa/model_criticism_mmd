...
#
#
# def test_any_distance_rbf_kernel_grad(resource_path_root):
#     array_obj = np.load(str(resource_path_root / 'eval_array.npz'))
#     x_train = torch.tensor(array_obj['x'][:100, :], requires_grad=True) + 1
#     y_train = torch.tensor(array_obj['y'][:100, :]) - 1
#     kernel_func_any = kernels_torch.AnyDistanceRBFKernelFunction(opt_sigma=False)
#     kernel_obj_any = kernel_func_any.compute_kernel_matrix(x=x_train, y=y_train)
#     grad_outputs = torch.ones_like(kernel_obj_any.k_xx)
#     grads = torch.autograd.grad(kernel_obj_any.k_xx, x_train, grad_outputs=grad_outputs)[0]
#     # check if grad exists
#     #print_graph(kernel_obj_any.k_xx.grad_fn)
...
#     test_any_distance_rbf_kernel_grad(Path('../../resources').absolute())
